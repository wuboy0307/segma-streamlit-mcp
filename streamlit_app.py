...
PROMPT_TEMPLATES = {
    "zh": {
        ...
        "🏷️ 標籤(對象的屬性 / 計算結果)": [
            ...
            ("〔衍生〕星座(生日 → 星座,巢狀 IIF)", "幫『客戶』建衍生(derive)標籤『<名稱,例如 星座>』,依生日欄位『<生日標籤,例如 date_of_birth>』的月份與日期,對應到 12 個西洋星座(用標準日期範圍,例如 牡羊座 3/21–4/19、金牛座 4/20–5/20…雙魚座 2/19–3/20)。請用巢狀 IIF 搭配 MONTH()、DAY() 這些公式函式寫(先 list_db_functions 確認函式與語法),不要用 SQL 的 CASE WHEN。"),
        ...
        "🎯 分群(篩出一群對象)": [
            ...
            ("上傳名單建分群(不用條件)", "建一個叫『<名稱,例如 黑五活動名單>』的分群,成員不用條件算,直接用我給的名單:對象是『客戶』分析主體,名單是 CSV,第一行是主鍵欄名 <customer_id>,底下一行一個人。內容:<把 CSV 貼在這裡,或給我檔案路徑>。(上傳型分群會把名單寫成資料倉裡的一張表,所以資料來源要能建表——postgres / greenplum / oracle / sqlserver / bigquery,而且要有 write schema;mysql / sybase 不支援。名單載進去之前這個分群是 0 人,不是全部人。)"),
            ("分群還沒建,先檢查名單", "我手上已經有一份名單,但分群還沒建。先幫我檢查這份名單對不對:對象是『客戶』分析主體,主鍵欄 <customer_id>,告訴我讀到幾筆、格式有沒有問題。這一步不要建分群、也不要碰資料倉(名單會先寄放在伺服器上幾個小時等分群來認領),檢查過了我再說要不要建分群把它掛上去。名單:<貼上 CSV>"),
            ("先上傳、看過數字再發布", "把這份名單上傳到『<分群名>』,但先不要進資料倉。先告訴我讀到幾筆、接受幾筆、修掉幾筆、幾筆重複,數字沒問題我再叫你發布。(『有幾筆對不到人』這時候還不知道——那要等發布、名單真的比對過資料倉之後才算得出來,所以不要在這一步跟我報這個數字。)名單:<貼上 CSV>"),
            ("名單裡有查無此人時怎麼辦", "『<分群名>』這個上傳名單分群,名單裡的人如果在『客戶』裡查無此人,請設成 <保留(預設,名單給誰就算誰) / 丟掉(只留對得到的,並回報丟了幾筆) / 整份退回(只要有一筆對不到就整份不收)>。(保留跟丟掉是每次讀取才套用,改了立刻生效、不用重傳;整份退回是上傳當下的關卡,過了之後就跟保留一樣。)"),
            ("追加名單(不覆蓋原本的)", "把這份名單追加到『<分群名>』,不要覆蓋原本已經在裡面的人——我要累積所有曾經符合過的人。名單:<貼上 CSV>"),
            ("看名單上傳歷史 / 少了誰", "『<分群名>』的名單上傳紀錄給我看:每次傳了幾筆、實際載入幾筆、有幾筆對不到人、對不到的長什麼樣,最近一次是成功還是失敗。"),
            ("分群落地成快照表", "把『<分群名>』這個『用條件算出來』的分群改成落地成一張快照表,並設 <每天凌晨 3 點> 重算一次。(落地之後這個分群回報的是最後一次建置寫進去的成員,不是每次查詢重算——所以同步送出去的也是那一份,建置之後才符合條件的人要等下次重算。上傳名單的分群不適用:它的名單本身就是一張表,設了會被忽略而且不會報錯,所以別對上傳型的下這個指令。)"),
        ],
        "📦 行動資料(整理輸出欄位)": [
            ...
            ("自訂 SQL 匯出(sql)", "建 sql 型行動資料『<名稱>』,建在資料來源『<資料來源名>』上,用這段我自己寫的 SQL 當輸出:<你的 SQL>(FROM 用真實存在的表 / 欄位,大小寫混合的名稱加雙引號)。"),
        ...
        "🔄 同步(把名單送出去)": [
            ...
            ("送出前先去重(dedup)", "把『客戶輪廓』同步到『電子報名單』,送出前先去掉重複——用輸出欄位 <email> 判斷是不是同一個人,同一個人只送一筆。"),
            ("重複的留最新那筆(dedup + 排序)", "把『客戶輪廓』同步到『電子報名單』,用輸出欄位 <email> 去重,同一個 email 有好幾筆時,留 <最後購買日> 最新的那一筆。"),
            ("指定送出的排序(sort)", "把『高價值客戶』分群同步到『每日名單匯出』,送出的資料照 <消費金額> 由大到小排。"),
            ("寫進資料表用 upsert(要一併去重)", "把『客戶輪廓』同步到『客戶主檔資料表』這個目的地,寫入模式用 upsert、以 <customer_id> 當比對鍵,並且用同一個欄位去重——同一個鍵有兩筆的話整批寫入會失敗,一列都寫不進去。"),
            ...
            ("失敗自動重試", "把『客戶輪廓』同步到『每日名單匯出』,執行失敗的話自動重試 <2> 次,每次間隔 <10> 分鐘。"),
        ...
        "🔍 查詢 / 探索(看看建了什麼)": [
            ...
            ("看它產生的 SQL", "把『<分群 / 行動資料 / 同步 / 指標 / 標籤 / 特徵商店 的名稱>』實際產生的 SQL 拉出來給我看,我要確認它到底是怎麼算的。"),
            ("看執行紀錄 / 失敗卡在哪", "『<行動資料 / 分群 / 同步 / 特徵商店 的名稱>』最近幾次執行給我看:什麼時候跑的、成功還失敗、下次排程什麼時候。最近一次如果失敗,把那次各個步驟的 log 拉出來,告訴我卡在哪一步。"),
        ],
        "🚀 一鍵完整流程": [
            ("從連線到分群一次做完",
    ...
    "en": {
        ...
        "🏷️ Trait (attributes / computed values)": [
            ...
            ("[derive] Zodiac (birthday → sign, nested IIF)", "Give 'Customer' a derive Trait '<name, e.g. Zodiac>' mapping the birthday '<birthday trait, e.g. date_of_birth>' (its month + day) to the 12 western zodiac signs by the standard date ranges (Aries 3/21–4/19, Taurus 4/20–5/20 … Pisces 2/19–3/20). Write it with nested IIF plus the MONTH() and DAY() functions (call list_db_functions first for the exact syntax), not a raw SQL CASE WHEN."),
        ...
        "🎯 Segment (a group)": [
            ...
            ("Upload a member list (no conditions)", "Build a Segment '<name, e.g. Black Friday List>' whose members come from a list I give you rather than from conditions. It's on the 'Customer' entity, and the list is CSV — first row is the primary key column name <customer_id>, one member per line. Here it is: <paste the CSV, or give me the file path>. (An uploaded segment stores its list as a table in the warehouse, so the data source has to be one Segma can create tables in — postgres, greenplum, oracle, sqlserver or bigquery, with its write schema set; mysql and sybase are refused. Until a list is loaded the segment has zero members, not everybody.)"),
            ("Check the list before the segment exists", "I have a list ready but the segment doesn't exist yet. Validate it first against the 'Customer' entity, primary key <customer_id>, and tell me how many rows it read and whether the format is wrong. Don't create the segment and don't touch the warehouse at this step (the list is parked on the server for a few hours waiting to be claimed) — if it checks out I'll tell you to create the segment and attach it. List: <paste the CSV>"),
            ("Upload first, publish after checking the counts", "Upload this list to '<segment name>' but don't load it into the warehouse yet. Tell me how many rows were read, accepted, trimmed and duplicated — I'll tell you to publish once the numbers look right. (How many matched nobody is NOT known at this point: that is counted when the list is published and compared against the warehouse, so don't report it at this step.) List: <paste the CSV>"),
            ("Decide what happens to keys that match nobody", "For the uploaded segment '<segment name>', when a key in the list matches nobody in 'Customer', use <keep (default — take the list as given) / drop (load only the keys that match, and report how many were dropped) / reject (refuse the whole list unless every key matches)>. (keep and drop are applied every time the segment is read, so switching takes effect immediately with no re-upload; reject is a gate applied once at load, and afterwards the segment reads like keep.)"),
            ("Append to the list instead of replacing it", "Append this list to '<segment name>' rather than replacing who is already in there — I'm accumulating everyone who has ever qualified. List: <paste the CSV>"),
            ("See the upload history and who was dropped", "Show me the upload history for '<segment name>': how many rows each upload received and loaded, how many keys matched nobody and what they look like, and whether the latest one succeeded or failed."),
            ("Store the segment as a snapshot table", "Store the condition-built Segment '<segment name>' as a snapshot table, rebuilt <every day at 3am>. (Once it's a snapshot the segment reports the members its last build wrote rather than recomputing on each read — so a Sync sends that same set, and anyone who qualifies after the build waits for the next one. This does not apply to an uploaded segment: its list is already a table, and asking for a snapshot there is ignored without raising an error, so don't send this for one.)"),
        ],
        "📦 Action Dataset (output columns)": [
            ...
            ("Custom-SQL export (sql)", "Build a sql Action Dataset '<name>' on data source '<data source name>' using this SQL I write: <your SQL> (FROM real tables/columns; double-quote mixed-case names)."),
        ...
        "🔄 Sync (send it out)": [
            ...
            ("Drop duplicates before sending (dedup)", "Sync 'Customer Profile' to the 'Newsletter List', dropping duplicates before it sends — treat rows sharing the same output column <email> as the same person, and send only one row each."),
            ("Keep the newest of each duplicate (dedup + ordering)", "Sync 'Customer Profile' to the 'Newsletter List', deduplicating on the output column <email>; when one email has several rows, keep the one with the most recent <last purchase date>."),
            ("Order the rows being sent (sort)", "Sync the 'High-Value Customers' segment to 'Daily List Export', with the rows sent ordered by <spend>, highest first."),
            ("Upsert into a table (dedup on the key)", "Sync 'Customer Profile' to the 'Customer Master Table' destination using upsert write mode keyed on <customer_id>, and deduplicate on that same column — two rows sharing a key make the whole write fail and nothing is written."),
            ...
            ("Retry a failed run", "Sync 'Customer Profile' to 'Daily List Export', retrying <2> more times if a run fails, waiting <10> minutes between attempts."),
        ...
        "🔍 Query / Explore (see what's built)": [
            ...
            ("See the SQL it compiles to", "Show me the SQL that '<name of a segment / action dataset / sync / metric / trait / feature store>' actually compiles to — I want to see how it's really being calculated."),
            ("See run history, and where a failure got stuck", "Show me the recent runs of '<name of an action dataset / segment / sync / feature store>': when each ran, whether it succeeded or failed, and when the next scheduled run is. If the latest one failed, pull that run's task logs and tell me which step it got stuck on."),
        ],
        "🚀 One-shot full flow": [
